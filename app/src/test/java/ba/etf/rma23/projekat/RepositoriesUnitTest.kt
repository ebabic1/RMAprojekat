...
class RepositoriesUnitTest {
    ...
    suspend fun obrisi(){
        ...
        var builder: Request.Builder = Request.Builder()
            .url(URL("https://rma23ws.onrender.com/account/" + AccountApiConfig.AccountGamesRepository.getHash() + "/game"))
            .delete()
    ...
    fun a0_pripremiPocetak() = runBlocking {
        AccountApiConfig.AccountGamesRepository.setHash("ca0ee672-440b-45b2-8a12-75b80f4fbdd3")
        obrisi()
    }

    @Test
    fun a1_getIgreFromAccount() = runBlocking {
        var res = AccountApiConfig.AccountGamesRepository.getSavedGames()
    ...
    fun a2_addOneGetSaved() = runBlocking {
        AccountApiConfig.AccountGamesRepository.saveGame(Game(24273,"Age of Empires: The Age of Kings","","",10.0,"","","","","","",listOf<UserImpression>()))
        AccountApiConfig.AccountGamesRepository.saveGame(Game(47076,"Age of Empires: Gold Edition","","",10.0,"","","","","","",listOf<UserImpression>()))
    ...
    fun a4_getGamesByName() = runBlocking {
        var res = IGDBApiConfig.GamesRepository.getGamesByName("Age of Empires")
    ...
    fun a5_getGamesSafe() = runBlocking {
        AccountApiConfig.AccountGamesRepository.setAge(10)
        var res = IGDBApiConfig.GamesRepository.getGamesSafe("Hitman")
        assertThat(res, CoreMatchers.not(CoreMatchers.hasItem<Game>(Matchers.hasProperty("id",CoreMatchers.equalTo(11157)))))

        var res2 = IGDBApiConfig.GamesRepository.getGamesByName("Hitman")
    ...
    fun a6_obrisiIgre() = runBlocking {
        var res= AccountApiConfig.AccountGamesRepository.getSavedGames()
        assertThat(res.size,CoreMatchers.equalTo(2))
        AccountApiConfig.AccountGamesRepository.removeGame(res.get(0).id)
        res= AccountApiConfig.AccountGamesRepository.getSavedGames()
    ...
    fun a7_dodajIgruUOmiljeneISortiraj() = runBlocking {
        AccountApiConfig.AccountGamesRepository.saveGame(Game(24273,"Age of Empires: The Age of Kings","","",10.0,"","","","","","",listOf<UserImpression>()))
        IGDBApiConfig.GamesRepository.getGamesByName("Age of Empires")
        var res = IGDBApiConfig.GamesRepository.sortGames()
